...
public class DispatcherServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Controller pageController =null; //PageController생성
			if("/member/list.do".equals(servletPath)) { //요청이(list.do) 들어온 servletPath와 리스트(메인화면)의 경로가 같은지 비교
				pageController = new MemberListController();//코드 변환
			}else if("/member/add.do".equals(servletPath)) { //신규등록을 하려면 요청이면
				pageController = new MemberAddController();	//add를 pc에 저장
